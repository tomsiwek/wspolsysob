...
//brzeg
 for (k=0; k<Ny; k++){
     ...
     for (y=0; y<Ny; y++)
     	 for (x=0; x<Nx; x++)
             V[k][0] += ro[k][0] * 1. / sqrt( (x-0)*(x-0) + (y-k)*(y-k) );
 }
 for (k=0; k<Ny; k++){
     V[k][Nx-1] = 0;
     for (y=0; y<Ny; y++)
         for (x=0; x<Nx; x++)
             V[k][Nx-1] += ro[k][Nx-1] * 1. / sqrt( (x-Nx+1)*(x-Nx+1) + (y-k)*(y-k) );
 }
 for (k=0; k<Nx; k++){
     V[0][k] = 0;
     for (y=0; y<Ny; y++)
         for (x=0; x<Nx; x++)
             V[0][k] += ro[0][k] * 1. / sqrt( (x-k)*(x-k) + (y-0)*(y-0) );
 }
 for (k=0; k<Nx; k++){
     V[Ny-1][k] = 0;
     for (y=0; y<Ny; y++)
         for (x=0; x<Nx; x++)
             V[Ny-1][k] += ro[Ny-1][k] * 1. / sqrt( (x-k)*(x-k) + (y-Ny+1)*(y-Ny+1) );
 }
//brzeg
 for (k=0; k<Ny; k++)
     V_NEW[k][0] = 0;
